...
if (!identical(config$profile, "full")) {
  stop("`90_run_full_patterned_sensitivity.R` requires `SPLIV_SIM_PROFILE=full`.")
}
spliv_sim_print_config(config)

log_file <- spliv_sim_new_log_file(paths, "90_run_full_patterned_sensitivity", config$profile)
spliv_sim_log_header(log_file, "90_run_full_patterned_sensitivity", config, package_info, make_patterned_scenarios(config))
...
spliv_sim_log(log_file, "Patterned full summary written to: ", summary_out$file)
